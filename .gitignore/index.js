...
const talkedRecentlyCalendrier = new Set()
var prefix = "/"; 
...
if (d1 == emotes[0] && d2 == emotes[0] && d3 == emotes[0]) {
  ...
  setTimeout(() => {
   ...
   if (message.content.toLocaleLowerCase() == ("/casinobonus")) {
    if (message.channel.type === "dm") return;
    if (message.channel.id !== '505337666028437515') return;
...
const noell3 = ["🌟"]
    ...
    var emotes = [noell1,noell2,noell3]
...
if (d1 == emotes[0] && d2 == emotes[0] && d3 == emotes[0]) {
      message.channel.send(`Bravo ${message.author}, tu as gagné ! Tu rempotes une surprise mystère qui sera dévoilé à la fin de la journée !`)
    } else
    if (d1 == emotes[2] && d2 == emotes[2] && d3 == emotes[2]) {
      message.channel.send(`Bravo ${message.author}, tu as gagné ! Tu rempotes une surprise mystère qui sera dévoilé à la fin de la journée !`)
    } else
    if (d1 == emotes[1] && d2 == emotes[1] && d3 == emotes[1]) {
      ...
      message.channel.send(`Perdu ${message.author}, retente ta chance dans 3h ! Crois moi, ça vaut le coup !`)
...
client.on('message', message =>{
 ...
 if (message.channel.type == 'dm') return;
 if (message.channel.id !== '518190701050069012') return;
    if (today.getDate() === 6 && today.getMonth() === 11) {
    message.delete()
    message.channel.send(`Tu viens d'ouvir la case n°6 de ton calendrier ! ${message.author}`)
    message.author.send("Voici le contenue de la sixième case de votre calendrier : Mon sixième est `` `` (un espace). Vous remportez également ~~un couteau suisse~~ un coca !")
